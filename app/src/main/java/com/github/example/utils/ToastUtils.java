...
public class ToastUtils {
    ...
    private static Snackbar normalToast;
    private static Snackbar gravityToast;
    ...
    private static Runnable run = new Runnable() {
        public void run() {
            if (normalToast != null) normalToast.dismiss();
            if (gravityToast != null) gravityToast.dismiss();
        }
    };
    ...
    private static void toast(Context context, CharSequence text, int duration) {
        ...
        if (normalToast == null) {
            normalToast = Snackbar.with(context);
        } else {
            normalToast.text(text);
        }
        handler.postDelayed(run, delayMillis);
        normalToast.showAnimation(true);
    }
    ...
    private static void toast(Context context, CharSequence text, int duration, int gravity, int xOffset, int yOffset) {
        ...
        if (gravityToast == null) {
            gravityToast = Snackbar.with(context);
        } else {
            gravityToast.text(text);
        }
        gravityToast.setGravity(gravity);
        handler.postDelayed(run, delayMillis);
        gravityToast.showAnimation(true);
    }
